...
def initialize_persistence() -> dict[str, object]:
    ...
    db.init_schema()
    persisted_environments = db.list_records("environments", Environment) if db.enabled else []
    if persisted_environments:
        _ENVIRONMENTS.clear()
...
def primary_environment_id() -> str:
    ...
    if PRIMARY_ENVIRONMENT_ID in _ENVIRONMENTS:
        return PRIMARY_ENVIRONMENT_ID
    if _ENVIRONMENTS:
        return next(iter(_ENVIRONMENTS))
    return PRIMARY_ENVIRONMENT_ID
...
def create_environment(name: str, environment_type: str, description: str, status: str = "active") -> Environment:
    ...
    _PLANS.setdefault(environment.environment_id, {})
    if db.enabled:
        db.upsert_many("environments", [environment])
    return environment


def update_environment(environment_id: str, name: str, environment_type: str, description: str, status: str) -> Environment | None:
    """Update one managed environment."""

    existing = _ENVIRONMENTS.get(environment_id)
    if existing is None:
        return None
    ...
    _ENVIRONMENTS[environment_id] = updated
    if db.enabled:
        db.upsert_many("environments", [updated])
    return updated
...
def delete_environment(environment_id: str) -> bool:
    ...
    _PLANS.pop(environment_id, None)
    ...
    for records in (
        ...
        _TRACKING_FINDINGS,
        _DEMO_REMEDIATIONS,
        _TRACKING_REMEDIATIONS,
        _DEMO_TELEMETRY_SUMMARIES,
        _TRACKING_TELEMETRY_SUMMARIES,
        _DEMO_ALERTS,
        _TRACKING_ALERTS,
        _DEMO_SECURITY_SIGNALS,
        _TRACKING_SECURITY_SIGNALS,
        _DEMO_INCIDENTS,
        _TRACKING_INCIDENTS,
        _DEMO_SERVICE_HEALTH,
        _TRACKING_SERVICE_HEALTH,
        _DEMO_DEPENDENCIES,
        _TRACKING_DEPENDENCIES,
        _DEMO_TELEMETRY_SOURCE_HEALTH,
        _TRACKING_TELEMETRY_SOURCE_HEALTH,
    ):
        _replace_environment_records(records, environment_id)

    _EXECUTION_RESULTS[:] = [result for result in _EXECUTION_RESULTS if result.environment_id != environment_id]
    _AUTOMATION_RUNS[:] = [run for run in _AUTOMATION_RUNS if run.environment_id != environment_id]

    if db.enabled:
        db.purge_environment(environment_id, keep_tables={"environments"})
        db.delete_record("environments", environment_id)
    return True
...
def save_tracking_finding(finding: Finding) -> Finding:
    """Persist one tracking finding generated by a safe runtime workflow."""

    _enable_tracking_mode()
    _TRACKING_FINDINGS[finding.id] = finding.model_copy(update={"score": calculate_finding_score(finding, None), "updated_at": utc_now()})
    _persist_environment_tracking(finding.environment_id)
    return _TRACKING_FINDINGS[finding.id]


def save_tracking_findings(findings: list[Finding]) -> list[Finding]:
    """Persist multiple tracking findings generated by safe runtime workflows."""

    return [save_tracking_finding(finding) for finding in findings]
...
def _unique_environment_id(base_environment_id: str) -> str:
    if base_environment_id not in _ENVIRONMENTS:
        return base_environment_id

    suffix = 2
    while f"{base_environment_id}-{suffix}" in _ENVIRONMENTS:
        suffix += 1
    return f"{base_environment_id}-{suffix}"
